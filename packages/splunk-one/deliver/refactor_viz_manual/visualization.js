/* eslint-disable */
/**
 * BGDHamp — refactor viz manual (vanilla AMD)
 *
 * Same implementation as splunkstuff_kpi_line with formatter keys for refactor_viz_manual.
 * Loads trend + hover from ./bgdhampTrendColors and ./bgdhampVizHoverMath (copies synced from repo visualizations/_shared at build).
 *
...
define([
    ...
    function debugEnabled() {
        try {
            return typeof window !== 'undefined' && window.REFACTOR_VIZ_MANUAL_DEBUG === true;
        } catch (e) {
            return false;
        }
    }
    ...
    if (typeof console !== 'undefined' && typeof console.info === 'function') {
        // One-time signpost; detailed traces need window.REFACTOR_VIZ_MANUAL_DEBUG = true (see file header).
        // eslint-disable-next-line no-console
        console.info(
            '[' + VIZ_ID + '] visualization loaded. Verbose logs: window.REFACTOR_VIZ_MANUAL_DEBUG = true (then refresh).'
        );
    }
        ...
        formatData: function (rawData) {
            ...
            if (idx < 0) {
                ...
                throw new SplunkVisualizationBase.VisualizationError(
                    'Refactor viz manual (BGDHamp) requires at least one all-numeric column (excluding _time).'
                );
            }
