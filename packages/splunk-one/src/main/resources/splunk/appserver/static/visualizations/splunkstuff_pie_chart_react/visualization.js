...
;// ./src/main/webapp/components/visualizations/PieChart.jsx


var DEFAULT_PALETTE = ['#01417F', '#DFA611', '#5CC05C', '#F47A22', '#9B6BFF', '#00A9D4', '#E85B79', '#8B9BB4'];
function applyTopN(slices, topN, otherLabel) {
  ...
  var otherSum = rest.reduce(function (sum, s) {
    return sum + s.value;
  }, 0);
  if (otherSum > 0) {
    keep.push({
...
function appendPieSlice(svg, cx, cy, r, startAngle, sweep, fill, stroke) {
  ...
  var d = "M".concat(cx, " ").concat(cy, " L").concat(x1.toFixed(2), " ").concat(y1.toFixed(2), " A").concat(r, " ").concat(r, " 0 ").concat(large, " 1 ").concat(x2.toFixed(2), " ").concat(y2.toFixed(2), " Z");
...
function PieChart(_ref) {
  var slices = _ref.slices,
    _ref$topN = _ref.topN,
    topN = _ref$topN === void 0 ? 5 : _ref$topN,
    _ref$otherLabel = _ref.otherLabel,
    otherLabel = _ref$otherLabel === void 0 ? 'Other' : _ref$otherLabel,
    _ref$showPercent = _ref.showPercent,
    showPercent = _ref$showPercent === void 0 ? true : _ref$showPercent,
    _ref$title = _ref.title,
    title = _ref$title === void 0 ? '' : _ref$title,
    _ref$background = _ref.background,
    background = _ref$background === void 0 ? '#1B2A41' : _ref$background,
  ...
  var shaped = (0,react.useMemo)(function () {
    return applyTopN(slices || [], topN, otherLabel);
  }, [slices, topN, otherLabel]);
  var total = (0,react.useMemo)(function () {
    return shaped.reduce(function (sum, s) {
      return sum + s.value;
    }, 0);
  }, [shaped]);
  var svgRef = react.useCallback(function (node) {
    if (!node || total <= 0) {
      return;
    }
    node.innerHTML = '';
    ...
    var r = 88;
    var angle = -Math.PI / 2;
    shaped.forEach(function (slice, i) {
      var sweep = slice.value / total * Math.PI * 2;
      if (sweep <= 0) {
        return;
      }
      appendPieSlice(node, cx, cy, r, angle, sweep, DEFAULT_PALETTE[i % DEFAULT_PALETTE.length], background);
      angle += sweep;
    });
  ...
  return /*#__PURE__*/react.createElement("div", {
    ...
    }, showPercent ? "".concat(slice.label, " \u2014 ").concat(slice.value.toLocaleString(), " (").concat(pct, "%)") : "".concat(slice.label, " \u2014 ").concat(slice.value.toLocaleString())));
  }))));
}
PieChart.propTypes = {
  slices: prop_types_default().arrayOf(prop_types_default().shape({
    label: (prop_types_default()).string.isRequired,
    value: (prop_types_default()).number.isRequired
  })),
  topN: prop_types_default().oneOfType([(prop_types_default()).number, (prop_types_default()).string]),
  otherLabel: (prop_types_default()).string,
  showPercent: (prop_types_default()).bool,
...
;// ./src/main/webapp/visualizations/splunkstuff_pie_chart_react/visualization.amd.jsx
/* eslint-disable */
...
var NS = 'display.visualizations.custom.so_BUI_pickulationts.splunkstuff_pie_chart_react.';
var roots = new WeakMap();
var VALUE_HINTS = ['value', 'count', 'sum', 'total', 'amount', 'pct', 'percent', 'avg', 'mean'];
var LABEL_HINTS = ['category', 'label', 'name', 'series', 'slice', 'status', 'host', 'field'];
function isNumericColumn(rawData, colIdx) {
  var col = rawData.columns[colIdx] || [];
  return col.some(function (cell) {
    return Number.isFinite(parseNum(cell));
  });
}
function hintMatch(name, hints) {
  return hints.some(function (hint) {
    return name === hint || name.indexOf(hint) >= 0;
  });
}
function pickColumns(rawData) {
  ...
  var labelIdx = -1;
  for (var c = 0; c < rawData.columns.length; c += 1) {
    var name = fieldName(fields, c).toLowerCase();
  ...
  if (valueIdx < 0) {
    valueIdx = rawData.columns.findIndex(function (_, c) {
      return isNumericColumn(rawData, c);
    });
  }
  for (var _c = 0; _c < rawData.columns.length; _c += 1) {
    var _name = fieldName(fields, _c).toLowerCase();
    if (_c !== valueIdx && hintMatch(_name, LABEL_HINTS) && !isNumericColumn(rawData, _c)) {
      labelIdx = _c;
      break;
    }
  }
  if (labelIdx < 0) {
    labelIdx = rawData.columns.findIndex(function (_, c) {
      return c !== valueIdx && !isNumericColumn(rawData, c);
    });
  }
  return {
    labelIdx: labelIdx,
...
function slicesFromRaw(rawData) {
  var picked = pickColumns(rawData);
  if (picked.valueIdx < 0) {
    return [];
  }
  var labels = picked.labelIdx >= 0 ? rawData.columns[picked.labelIdx] || [] : [];
  var values = rawData.columns[picked.valueIdx] || [];
  var byLabel = {};
  var order = [];
  for (var i = 0; i < values.length; i += 1) {
    var value = parseNum(values[i]);
    if (!Number.isFinite(value) || value < 0) {
      continue;
    }
    var label = picked.labelIdx >= 0 ? String(cellValue(labels[i]) || '') : "Row ".concat(i + 1);
    if (!label) {
      label = '(blank)';
    }
...
function mount(el, props) {
  ...
  root.render(/*#__PURE__*/react.createElement(PieChart, props));
}
/* harmony default export */ const visualization_amd = (SplunkVisualizationBase_default().extend({
  ...
  formatData: function formatData(rawData) {
    return {
      slices: rawData && rawData.columns ? slicesFromRaw(rawData) : []
    };
  },
